...
public class GenUtils {
    ...
    public static void initTable(GenTable genTable, String operName) {
        ...
        String cleanComment = parseTableCommentMarkers(genTable, tableComment);
        genTable.setFunctionName(replaceText(cleanComment));
     ...
     * @return 移除标记后的表备注
     */
    private static String parseTableCommentMarkers(GenTable genTable, String tableComment) {
        if (StrUtil.isEmpty(tableComment)) {
            genTable.setIsInherit("1"); // 默认继承
            return tableComment;
        }

        String cleanComment = tableComment;

        // 检查 [不继承] 标记
        if (tableComment.contains("[不继承]")) {
            genTable.setIsInherit("0");
            cleanComment = cleanComment.replace("[不继承]", "").trim();
        }
        // 检查 [继承] 标记（显式指定继承，默认行为）
        else if (tableComment.contains("[继承]")) {
            genTable.setIsInherit("1");
            cleanComment = cleanComment.replace("[继承]", "").trim();
        }
        // 默认继承
        else {
            genTable.setIsInherit("1");
    ...
    public static void initColumnField(GenTableColumn column, GenTable table) {
        ...
        // 最后解析字段注释中的控制标记，覆盖智能设置的属性
        parseColumnCommentMarkers(column);
    }
     ...
     * @param column 字段信息
     */
    private static void parseColumnCommentMarkers(GenTableColumn column) {
        ...
        // 处理列表显示标记
        if (columnComment.contains("[列表]") || columnComment.contains("[显示]")) {
            column.setIsList("1");
        } else if (columnComment.contains("[不列表]") || columnComment.contains("[不显示]")) {
            column.setIsList("0");
        }

        // 处理查询标记
        if (columnComment.contains("[查询]")) {
            column.setIsQuery("1");
        } else if (columnComment.contains("[不查询]")) {
            column.setIsQuery("0");
        }

        // 处理必填标记
        if (columnComment.contains("[必填]")) {
            column.setIsRequired("1");
        } else if (columnComment.contains("[选填]")) {
    ...
    private static int getListDisplayPriority(String columnName) {
        // 创建时间最高优先级
        if (StrUtil.equalsAnyIgnoreCase(columnName, "create_time", "createTime", "created_at", "createdAt",
    ...
    private static void setJavaTypeAndHtmlType(GenTableColumn column, String dataType) {
        if (arraysContains(GenConstants.COLUMNTYPE_STR, dataType)
            ...
            String[] str = StrUtil
                    .splitToArray(StrUtil.sub(column.getColumnType(), 0, column.getColumnType().indexOf("(")), " ");
            if (str != null && str.length == 2 && StrUtil.equals(str[1], "decimal")) {
                column.setJavaType(GenConstants.TYPE_BIGDECIMAL);
            }
            // 如果是整形
            else if (str != null && str.length == 1
                    && (StrUtil.equals(str[0], "bigint") || StrUtil.equals(str[0], "int"))) {
    ...
    private static void setSmartFieldProperties(GenTableColumn column, String columnName) {
        ...
        // 修改时间字段 - 默认不显示在列表中
        if (StrUtil.equalsAnyIgnoreCase(columnName, "update_time", "updateTime", "updated_at", "updatedAt",
                "gmt_modified", "gmtModified", "modify_time", "modifyTime")) {
        ...
        // 创建人字段 - 默认不显示在列表中
        if (StrUtil.equalsAnyIgnoreCase(columnName, "create_by", "createBy", "created_by", "createdBy", "creator",
        ...
        // 修改人字段 - 默认不显示在列表中
        if (StrUtil.equalsAnyIgnoreCase(columnName, "update_by", "updateBy", "updated_by", "updatedBy", "modifier",
                "update_user", "updateUser", "modify_by", "modifyBy")) {
        ...
        // 备注字段 - 默认不显示在列表中
        if (StrUtil.equalsAnyIgnoreCase(columnName, "remark", "remarks", "note", "notes", "comment", "comments",
                "description", "desc")) {
            column.setIsInsert(GenConstants.REQUIRE);
            column.setIsEdit(GenConstants.REQUIRE);
