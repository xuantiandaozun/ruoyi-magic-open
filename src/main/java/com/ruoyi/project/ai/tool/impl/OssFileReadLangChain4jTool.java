...
public class OssFileReadLangChain4jTool implements LangChain4jTool {
    ...
    public String execute(Map<String, Object> parameters) {
        ...
        if (StrUtil.isBlank(ossUrl)) {
            log.info("[OssFileReadTool] OSS URL为空，跳过读取，返回继续执行指示");
            return ToolExecutionResult.querySuccess(
                    "【README内容不可用】请直接根据项目的description、stars、language等基础信息进行分析，不要再次尝试读取文件。",
                    "README地址为空，已跳过读取。请使用项目描述信息继续完成分析任务。");
        }

        if (!ossUrl.startsWith("http://") && !ossUrl.startsWith("https://")) {
            log.warn("[OssFileReadTool] OSS URL不合法，跳过读取: url={}", ossUrl);
            return ToolExecutionResult.querySuccess(
                    "【README内容不可用】提供的URL格式不正确（非http/https开头）。请直接根据项目的description、stars、language等基础信息进行分析，不要再次尝试读取文件。",
                    "URL格式不合法，已跳过读取。请使用项目描述信息继续完成分析任务。");
        }
