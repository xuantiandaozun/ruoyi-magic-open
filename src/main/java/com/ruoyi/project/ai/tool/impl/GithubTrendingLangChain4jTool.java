...
public class GithubTrendingLangChain4jTool implements LangChain4jTool {
    ...
    public ToolSpecification getToolSpecification() {
        ...
        JsonObjectSchema parametersSchema = JsonObjectSchema.builder()
                ...
                .addStringProperty("timeRange",
                        "时间范围:today(今日)、week(本周)、month(本月)、auto(自动降级,默认)。auto模式下优先查询今日,若无结果则自动尝试本周、本月")
    ...
    public String execute(Map<String, Object> parameters) {
        // 获取参数
        Integer limit = parameters.get("limit") != null ? Integer.parseInt(parameters.get("limit").toString()) : 10;
        String language = (String) parameters.get("language");
        Integer minStars = parameters.get("minStars") != null ? Integer.parseInt(parameters.get("minStars").toString())
                : null;
        Integer maxStars = parameters.get("maxStars") != null ? Integer.parseInt(parameters.get("maxStars").toString())
                : null;
        String orderBy = (String) parameters.get("orderBy");
        Boolean hasReadme = parameters.get("hasReadme") != null
                ? Boolean.parseBoolean(parameters.get("hasReadme").toString())
                : null;
        String timeRange = parameters.get("timeRange") != null ? parameters.get("timeRange").toString() : "auto";
        Boolean includeGenerated = parameters.get("includeGenerated") != null
                ? Boolean.parseBoolean(parameters.get("includeGenerated").toString())
                : false;

        log.info(
                "[GithubTrendingTool] 开始查询GitHub趋势仓库, params: limit={}, language={}, minStars={}, maxStars={}, timeRange={}, includeGenerated={}",
        ...
        if ("auto".equalsIgnoreCase(timeRange)) {
            // 自动降级模式：今日 -> 本周 -> 本月
            repositories = queryRepositories("today", language, minStars, maxStars, hasReadme, orderBy, generatedRepos,
                    limit * 3);
            ...
            if (repositories.isEmpty() && StrUtil.isNotBlank(language)) {
                ...
                repositories = queryRepositories("today", null, minStars, maxStars, hasReadme, orderBy, generatedRepos,
                        limit * 3);
            }

            if (repositories.size() < limit) {
                List<GithubTrending> weekRepos = queryRepositories("week", language, minStars, maxStars, hasReadme,
                        orderBy, generatedRepos, limit * 3);
                // 如果指定了语言但查不到,尝试不指定语言再查一次
                if (weekRepos.isEmpty() && StrUtil.isNotBlank(language)) {
                    log.warn("[GithubTrendingTool] 本周指定语言{}查不到数据,尝试查询所有语言", language);
                    weekRepos = queryRepositories("week", null, minStars, maxStars, hasReadme, orderBy, generatedRepos,
                            limit * 3);
                }
                // 合并并去重
            ...
            if (repositories.size() < limit) {
                List<GithubTrending> monthRepos = queryRepositories("month", language, minStars, maxStars, hasReadme,
                        orderBy, generatedRepos, limit * 3);
                // 如果指定了语言但查不到,尝试不指定语言再查一次
                if (monthRepos.isEmpty() && StrUtil.isNotBlank(language)) {
                    log.warn("[GithubTrendingTool] 本月指定语言{}查不到数据,尝试查询所有语言", language);
                    monthRepos = queryRepositories("month", null, minStars, maxStars, hasReadme, orderBy,
                            generatedRepos, limit * 3);
                }
                // 合并并去重
            ...
            // 指定时间范围
            repositories = queryRepositories(timeRange, language, minStars, maxStars, hasReadme, orderBy,
                    generatedRepos, limit * 3);
            // 如果指定了语言但查不到,尝试不指定语言再查一次
            if (repositories.isEmpty() && StrUtil.isNotBlank(language)) {
                log.warn("[GithubTrendingTool] 指定语言{}查不到数据,尝试查询所有语言", language);
                repositories = queryRepositories(timeRange, null, minStars, maxStars, hasReadme, orderBy,
                        generatedRepos, limit * 3);
            }
        ...
        if (StrUtil.isNotBlank(language))
            filterInfo.append("语言：").append(language).append(" ");
        if (minStars != null)
            filterInfo.append("最小星数：").append(minStars).append(" ");
        if (maxStars != null)
            filterInfo.append("最大星数：").append(maxStars).append(" ");
        ...
        for (int i = 0; i < repositories.size(); i++) {
            ...
            // 添加README文件相关信息（始终显示，明确标识是否可用）
            String readmePath = repo.getReadmePath();
            if (StrUtil.isNotBlank(readmePath)
                    && (readmePath.startsWith("http://") || readmePath.startsWith("https://"))) {
                result.append(String.format("   readmePath: %s\n", readmePath));
                result.append("   README可读取: 是（可使用oss_file_read工具读取上述地址）\n");
            } else if (StrUtil.isNotBlank(readmePath)) {
                result.append(String.format("   readmePath: %s（格式不正确，非http/https开头）\n", readmePath));
                result.append("   README可读取: 否（地址格式无效，请跳过读取）\n");
            } else {
                result.append("   readmePath: 无\n");
                result.append("   README可读取: 否（无README地址，请跳过读取，使用项目描述进行分析）\n");
            }
            // AI翻译README（已弃用，不再推荐使用）
            if (StrUtil.isNotBlank(repo.getAiReadmePath())) {
                result.append(String.format("   aiReadmePath: %s（已弃用，请勿使用）\n", repo.getAiReadmePath()));
            }
     ...
     * @return 过滤后的仓库列表
     */
    private List<GithubTrending> filterAlreadyGenerated(List<GithubTrending> repositories,
            List<GeneratedRepoInfo> generatedRepos) {
    ...
    private List<GithubTrending> queryRepositories(String timeRange, String language, Integer minStars,
        ...
        QueryWrapper qw = QueryWrapper.create()
                ...
                .where(new QueryColumn("first_trending_date")
                        .ge(startDate.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))))
                .and(new QueryColumn("first_trending_date")
                        .le(endDate.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))));
