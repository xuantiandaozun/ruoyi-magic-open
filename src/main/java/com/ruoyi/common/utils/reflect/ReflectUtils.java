...
public class ReflectUtils
{
            ...
            for (int i = 0; i < cs.length; i++)
            {
                if (args[i] != null && !args[i].getClass().equals(cs[i]))
                {
                    ...
                    else if (cs[i] == Date.class)
                    {
                        ...
                        {
                            args[i] = DateUtil.date(((Double) args[i]).longValue());
                        }
                    }
    ...
    /**
     * 改变private/protected的成员变量为public，尽量不调用实际��动的语句，避免JDK的SecurityManager抱怨。
     */
    ...
    public static Class getClassGenricType(final Class clazz, final int index)
    {
        ...
        {
            logger.debug(clazz.getSimpleName() + "的父类不是泛型类型");
            return Object.class;
        }
        ...
        {
            logger.debug(clazz.getSimpleName() + "不是泛型类型");
            return Object.class;
        }
